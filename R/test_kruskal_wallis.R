...
#' @param method string, optional. the specific variation of the test to use. Default is "chi2". Options are: : "chi2", "kw-gamma", "kw-gamma-chi2", "kw-beta", "kw-beta-f", "wallace-f1", "wallace-f2", "wallace-f3", "wallace-beta1", "wallace-beta2", "wallace-beta3", "ids"
#' @param tiescorr boolean, optional. use of a ties correction. Default is True.
#'
#' @returns
...
#'
#' @export
ts_kruskal_wallis <- function(catField, ordField, categories=NULL, levels=NULL,
                               method=c("chi2", "kw-gamma", "kw-gamma-chi2", "kw-beta", "kw-beta-f", "wallace-I-beta", "wallace-II-beta", "wallace-III-beta", "wallace-I-f", "wallace-II-f", "wallace-III-f", "iman"),
                               tiescorr=c(TRUE, FALSE)
) {
  
  #default ties correction to TRUE
  if (length(tiescorr)>1) {tiescorr=tiescorr[1]}
  #default method to "chi2"
  ...
  Var = 2 * (k - 1) - (2 * (3 * k**2 - 6 * k + n * (2 * k**2 - 6 * k + 1))) / (5 * n * (n + 1)) - 6 / 5 * sum(1/nc)
  mu = k - 1
  
  if (method=="chi2"){
    #kw chi2 appr
    df = mu
    pValue = pchisq(h, df, lower.tail = FALSE)
    
    res <- data.frame(n, h, df, pValue)
    colnames(res) = c("n", "h", "df", "p-value")
  }
  
  else if (method == "kw-gamma"){
    #kw gamma appr
    bet = Var / mu
    alp = mu**2 / Var
    
    p = 1 - pgamma(q=h, shape=alp, scale=bet)
    
    res <- data.frame(n, h, alp, bet, p)
    colnames(res) = c("n", "h", "alpha", "beta", "p-value")        
  }
  
  else if (method == "kw-gamma-chi2"){
    #kw gamma chi appr
    chi2Val = 2 * mu / Var * h
    df = 2 * mu**2 / Var
    p = pchisq(chi2Val, df, lower.tail = FALSE)
    
    res <- data.frame(n, h, chi2Val, df, p)
    colnames(res) = c("n", "h", "chi2 statistic", "df", "p-value")
  }
  
  else if (method == "kw-beta" || method == "kw-beta-f"){
    #kw beta appr and
    ...
    if (method == "kw-beta"){
      ...
      alp = df1 * 0.5
      bet = df2 * 0.5
      p = 1 - pgamma(q=h/m, shape=alp, scale=1/bet)
      
      res <- data.frame(n, h, h/m, alp, bet, p)
      colnames(res) = c("n", "h", "B2", "alpha", "beta", "p-value") 
    }
    else{
      #kw beta F appr
      fVal = h * (m - mu) / (mu * (m - h))
      p = 1 - pf(q=fVal, df1, df2, lower.tail = TRUE)
      
      res <- data.frame(n, h, fVal, df1, df2, p)
      colnames(res) = c("n", "h", "F statistic", "df1", "df2", "p-value") 
    }
    
  }
  
  else{
    if (method == "wallace-beta1" || method == "wallace-f1"){
      d = ((n - k) * (k - 1) - Var) / (0.5 * (n - 1 )*Var)}
    else if (method == "wallace-beta2" || method == "wallace-f2"){
      d = 1 - 6 * (n + 1) / (5 * (n - 1) * (n + 1.2))}
    else if (method == "wallace-beta3" || method == "wallace-f3"){
      d = 1}
    
    if (method %in% c("wallace-f1", "wallace-f2", "wallace-f3", "wallace-beta1", "wallace-beta2", "wallace-beta3")){
      df1 = (k - 1) * d
      df2 = (n - k) * d}
    
    if (method %in% c("wallace-f1", "wallace-f2", "wallace-f3")){
      fw = (n - k) * h / ((k - 1) * (n - 1 - h))
      p = 1 - pf(q=fw, df1, df2, lower.tail = TRUE)
      
      res <- data.frame(n, h, fw, df1, df2, p)
      colnames(res) = c("n", "h", "F statistic", "df1", "df2", "p-value") 
      
    }
    
    else if (method %in% c("wallace-beta1", "wallace-beta2", "wallace-beta3")){
      b = h / (n - 1)
      alp = df1 * 0.5
      bet = df2 * 0.5
      
      p = 1 - pbeta(q=b, shape1=alp, shape2=bet)
      
      res <- data.frame(n, h, b, alp, bet, p)
      colnames(res) = c("n", "h", "B2", "alpha", "beta", "p-value") 
      ...
      fw = (n - k) * h / ((k - 1) * (n - 1 - h))                        
      vi = aggregate(dfr$r, by=list(group=dfr$catField), FUN=var)$x
